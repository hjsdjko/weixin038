...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussyuyueyishengCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussyuyueyishengCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussyuyueyisheng', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussyuyueyisheng');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussyuyueyisheng');
			...
			async init(){
				...
				let res = await this.$api.info('discussyuyueyisheng', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussyuyueyisheng', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussyuyueyisheng/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
