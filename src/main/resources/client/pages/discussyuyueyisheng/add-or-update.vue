	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussyuyueyisheng`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussyuyueyisheng`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussyuyueyisheng`, this.ruleForm);
						}else{
							await this.$api.add(`discussyuyueyisheng`, this.ruleForm);
						}
						uni.setStorageSync('discussyuyueyishengCleanType',true);
